    ...
    <div class="card mt-4">
        <div class="card-body">
            <form action="{{ route('admin.announcements.update', $announcement->id) }}" method="POST">
                ...
                @php
                    $roles = is_array($announcement->target_roles) ? $announcement->target_roles : json_decode($announcement->target_roles, true) ?? [];
                @endphp
                <div class="mb-3">
                    <label class="form-label">Target Role</label>
                    <div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input role-checkbox" type="checkbox" id="role_all" name="target_roles[]" value="all" {{ (is_array(old('target_roles', $roles)) && in_array('all', old('target_roles', $roles))) ? 'checked' : '' }}>
                            <label class="form-check-label" for="role_all">Semua (All)</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input role-checkbox" type="checkbox" id="role_siswa" name="target_roles[]" value="siswa" {{ (is_array(old('target_roles', $roles)) && in_array('siswa', old('target_roles', $roles))) ? 'checked' : '' }}>
                            <label class="form-check-label" for="role_siswa">Siswa</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input role-checkbox" type="checkbox" id="role_guru" name="target_roles[]" value="guru" {{ (is_array(old('target_roles', $roles)) && in_array('guru', old('target_roles', $roles))) ? 'checked' : '' }}>
                            <label class="form-check-label" for="role_guru">Guru</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input role-checkbox" type="checkbox" id="role_ortu" name="target_roles[]" value="ortu" {{ (is_array(old('target_roles', $roles)) && in_array('ortu', old('target_roles', $roles))) ? 'checked' : '' }}>
                            <label class="form-check-label" for="role_ortu">Orang Tua</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input role-checkbox" type="checkbox" id="role_tu" name="target_roles[]" value="tu" {{ (is_array(old('target_roles', $roles)) && in_array('tu', old('target_roles', $roles))) ? 'checked' : '' }}>
                            <label class="form-check-label" for="role_tu">TU</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input role-checkbox" type="checkbox" id="role_kepsek" name="target_roles[]" value="kepsek" {{ (is_array(old('target_roles', $roles)) && in_array('kepsek', old('target_roles', $roles))) ? 'checked' : '' }}>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="datetime-local" class="form-control @error('expired_at') is-invalid @enderror" id="expired_at" name="expired_at" value="{{ old('expired_at', $announcement->expired_at ? $announcement->expired_at->format('Y-m-d\TH:i') : '') }}">
